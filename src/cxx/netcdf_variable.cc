...
namespace Obs2Ioda {
    ...
    flattenCharArray(
        ...
        for (size_t i = 0; i < numStrings; ++i) {
            size_t len =
                    std::min(std::strlen(values[i]), stringLen - 1);
            // leave room for null terminator
            std::memcpy(&flattened[i * stringLen], values[i], len);
            flattened[i * stringLen + len] = '\0';
    ...
    int netcdfAddVar(
        ...
        try {
            ...
            bool compressible =
                    netcdfDataType != NC_STRING && netcdfDataType !=
                    NC_CHAR;
            ...
            if (compressible && !dims.empty() && zlibSettings->enabled) {
                ...
                var.setCompression(
                    zlibSettings->shuffle, zlibSettings->deflate,
    ...
    template<typename T, bool netcdfChar = false> int netcdfPutVar(
        int netcdfID, const char *groupName, const char *varName,
        const T *values
    ) {
        try {
            ...
            validateNetcdfDataType<T>(
                ...
                std::string(varName) + "': expected " + std::string(
                    typeid(T).name()
