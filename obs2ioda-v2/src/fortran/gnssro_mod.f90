...
status = netcdfAddVar(ncid, 'latitude', NF90_FLOAT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_lat, "units",  "degree_north" ))
status = netcdfPutAtt(ncid, "units", "degree_north", varName="latitude", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_lat, 0, real(r_missing)))
status = netcdfSetFill(ncid, "latitude", 1, real(r_missing), groupName="MetaData")
!call check( nf90_def_var(grpid_metadata, "longitude",     NF90_FLOAT, nlocs_dimid, varid_lon) )
status = netcdfAddVar(ncid, 'longitude', NF90_FLOAT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_lon, "units",  "degree_east" ))
status = netcdfPutAtt(ncid, "units", "degree_east", varName="longitude", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_lon, 0, real(r_missing)))
status = netcdfSetFill(ncid, "longitude", 1, real(r_missing), groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "time",          NF90_FLOAT, nlocs_dimid, varid_time) )
status = netcdfAddVar(ncid, 'time', NF90_FLOAT, 1, ['nlocs'], groupName='MetaData')

!call check( nf90_put_att(grpid_metadata, varid_time, "longname", "time offset to analysis time" ))
status = netcdfPutAtt(ncid, "longname", "time offset to analysis time", varName="time", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_time, "units", "hour" ))
status = netcdfPutAtt(ncid, "units", "hour", varName="time", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_time, 0, real(r_missing)))
status = netcdfSetFill(ncid, "time", 1, real(r_missing), groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "dateTime",     NF90_INT64, nlocs_dimid, varid_epochtime) )
status = netcdfAddVar(ncid, 'dateTime', NF90_INT64, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_epochtime, "units","seconds since 1970-01-01T00:00:00Z" ))
status = netcdfPutAtt(ncid, "units", "seconds since 1970-01-01T00:00:00Z", varName="dateTime", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_epochtime, 0, i64_missing ))
status = netcdfSetFill(ncid, "dateTime", 1, i64_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "record_number",   NF90_INT, nlocs_dimid, varid_recn))
status = netcdfAddVar(ncid, 'record_number', NF90_INT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_recn, "longname", "GNSS RO profile identifier" ))
status = netcdfPutAtt(ncid, "longname", "GNSS RO profile identifier", varName="record_number", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_recn, "units",  "1" ))
status = netcdfPutAtt(ncid, "units", "1", varName="record_number", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_recn, 0, i_missing ))
status = netcdfSetFill(ncid, "record_number", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "gnss_sat_class",  NF90_INT, nlocs_dimid, varid_sclf))
status = netcdfAddVar(ncid, 'gnss_sat_class', NF90_INT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_sclf, "longname", "GNSS satellite classification, &
!                                                     &e.g., 401=GPS, 402=GLONASS" ))
status = netcdfPutAtt(ncid, "longname", "GNSS satellite classification, e.g., 401=GPS, 402=GLONASS", varName="&
gnss_sat_class", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_sclf, "units",  "1" ))
status = netcdfPutAtt(ncid, "units", "1", varName="gnss_sat_class", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_sclf, 0, i_missing ))
status = netcdfSetFill(ncid, "gnss_sat_class", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "reference_sat_id", NF90_INT, nlocs_dimid, varid_ptid))
status = netcdfAddVar(ncid, 'reference_sat_id', NF90_INT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_ptid, "longname", "GNSS satellite transmitter identifier (1-32)" ))
status = netcdfPutAtt(ncid, "longname", "GNSS satellite transmitter identifier (1-32)", varName="reference_sat_id", &
groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_ptid, "units",  "1" ))
status = netcdfPutAtt(ncid, "units", "1", varName="reference_sat_id", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_ptid, 0, i_missing ))
status = netcdfSetFill(ncid, "reference_sat_id", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "occulting_sat_id", NF90_INT, nlocs_dimid, varid_said))
status = netcdfAddVar(ncid, 'occulting_sat_id', NF90_INT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_said, "longname", "Low Earth Orbit satellite identifier, e.g., COSMIC2=750-755" ))
status = netcdfPutAtt(ncid, "longname", "Low Earth Orbit satellite identifier, e.g., COSMIC2=750-755", varName="&
occulting_sat_id", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_said, "units",  "1" ))
status = netcdfPutAtt(ncid, "units", "1", varName="occulting_sat_id", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_said, 0, i_missing ))
status = netcdfSetFill(ncid, "occulting_sat_id", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "occulting_sat_is", NF90_INT, nlocs_dimid, varid_siid))
status = netcdfAddVar(ncid, 'occulting_sat_is', NF90_INT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_siid, "longname", "satellite instrument"))
status = netcdfPutAtt(ncid, "longname", "satellite instrument", varName="occulting_sat_is", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_siid, "units",  "1" ))
status = netcdfPutAtt(ncid, "units", "1", varName="occulting_sat_is", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_siid, 0, i_missing ))
status = netcdfSetFill(ncid, "occulting_sat_is", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "ascending_flag",  NF90_INT, nlocs_dimid, varid_asce))
status = netcdfAddVar(ncid, 'ascending_flag', NF90_INT, 1, ['nlocs'], groupName='MetaData')
...
status = netcdfPutAtt(ncid, "units", "1", varName="ascending_flag", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_asce, 0, i_missing ))
status = netcdfSetFill(ncid, "ascending_flag", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_metadata, "process_center",NF90_INT,nlocs_dimid, varid_ogce))
status = netcdfAddVar(ncid, 'process_center', NF90_INT, 1, ['nlocs'], groupName='MetaData')
...
status = netcdfPutAtt(ncid, "longname", "originally data processing_center, e.g., 60 for UCAR, 94 for DMI, 78 for GFZ", &
varName="process_center", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_ogce, "units",  "1" ))
status = netcdfPutAtt(ncid, "units", "1", varName="process_center", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_ogce, 0, i_missing ))
status = netcdfSetFill(ncid, "process_center", 1, i_missing, groupName="MetaData")
!
!call check( nf90_def_var(grpid_obsvalue, "refractivity", NF90_FLOAT, nlocs_dimid, varid_ref) )
status = netcdfAddVar(ncid, 'refractivity', NF90_FLOAT, 1, ['nlocs'], groupName='ObsValue')
!call check( nf90_put_att(grpid_obsvalue, varid_ref, "longname", "Atmospheric refractivity" ))
status = netcdfPutAtt(ncid, "longname", "Atmospheric refractivity", varName="refractivity", groupName="ObsValue")
...
status = netcdfAddVar(ncid, 'refractivity', NF90_FLOAT, 1, ['nlocs'], groupName='ObsError')
!call check( nf90_put_att(grpid_obserror, varid_refoe, "longname", "Input error in atmospheric refractivity" ))
status = netcdfPutAtt(ncid, "longname", "Input error in atmospheric refractivity", varName="refractivity", groupName="ObsError")
...
status = netcdfAddVar(ncid, 'altitude', NF90_FLOAT, 1, ['nlocs'], groupName='MetaData')
!call check( nf90_put_att(grpid_metadata, varid_msl, "longname", "Geometric altitude" ))
status = netcdfPutAtt(ncid, "longname", "Geometric altitude", varName="altitude", groupName="MetaData")
!call check( nf90_put_att(grpid_metadata, varid_msl, "units", "m" ))
status = netcdfPutAtt(ncid, "units", "m", varName="altitude", groupName="MetaData")
!call check( nf90_def_var_fill(grpid_metadata, varid_msl, 0, real(r_missing) ))
status = netcdfSetFill(ncid, "altitude", 1, real(r_missing), groupName="MetaData")
!
