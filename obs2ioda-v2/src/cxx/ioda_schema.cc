...
IodaSchema::IodaSchema(YAML::Node schema) {
    for (auto attribute: schema["Attributes"]) {
        if (attribute["Attribute"]) {
            ...
            iodaAttribute->name = attribute["Attribute"].begin()->as<
                std::string>();
            for (auto attr: attribute["Attribute"]) {
                this->attributes[attr.as<std::string>()] =
                        iodaAttribute;
            }
        }
    }
    for (auto variable: schema["Variables"]) {
        if (variable["Variable"]) {
            std::shared_ptr<IodaVariable> iodaVariable =
                    std::make_shared<IodaVariable>();
            iodaVariable->name = variable["Variable"].begin()->as<
                std::string>();
    ...
    for (auto dimension: schema["Dimensions"]) {
        if (dimension["Dimension"]) {
            ...
            auto iodaVariable = std::make_shared<IodaVariable>();
            iodaDimension->name = dimension["Dimension"].begin()->as<
                std::string>();
            iodaVariable->name = iodaDimension->name;
            for (auto dim: dimension["Dimension"]) {
                this->dimensions[dim.as<std::string>()] = iodaDimension;
                this->variables[dim.as<std::string>()] = iodaVariable;
            }
        }
    }
...
std::shared_ptr<IodaAttribute> IodaSchema::getAttribute(
...
std::shared_ptr<IodaVariable> IodaSchema::getVariable(
    const std::string &name
) {
    std::shared_ptr<IodaVariable> iodaVariable;
...
std::shared_ptr<IodaDimension> IodaSchema::getDimension(
    const std::string &name
) {
...
std::shared_ptr<IodaGroup>
IodaSchema::getGroup(const std::string &name) {
